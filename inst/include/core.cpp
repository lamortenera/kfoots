...
static inline void fitNBs_core(Vec<int> values, Mat<double> tposteriors, Vec<double> mus, Vec<double> rs, double tol=1e-8, int nthreads=1){
    ...
    for (int mod = 0; mod < nmod; ++mod){
        ...
        fitNB_core(values, tposteriors.getCol(mod), &mus[mod], &rs[mod], rs[mod], tol=tol, nthreads=nthreads_inner);
    }
}

static inline void fitNBs_1r_core(Vec<int> values, Mat<double> posteriors, Vec<double> mus, double* r, double tol=1e-8, int nthreads=1){
