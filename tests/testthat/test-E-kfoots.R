                    ...
                    for (maxiter in maxiters){
                        for (nthreads in nthreadss){
                            for (split4speed in c(TRUE, FALSE)){
                                #cat(k, init, nbtype, maxiter, length(seqlens), nthreads, "\n")
                                hmm <- kfoots(counts, k, maxiter=maxiter, nbtype=nbtype, init=init, seqlens=seqlens, nthreads=nthreads, framework="HMM", verbose=F, split4speed=split4speed)
                                models <- hmm$models
                                expect_true(modelsAreOk(models, k, nrow(counts), nbtype))
                                trans <- hmm$trans
